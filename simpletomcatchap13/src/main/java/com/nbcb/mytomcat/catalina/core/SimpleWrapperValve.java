...
import com.nbcb.mytomcat.catalina.filter.ApplicationFilterFactory;
import com.nbcb.mytomcat.catalina.filter.SimpleApplicationFilterChain;
...
public class SimpleWrapperValve implements Valve, Contained {
    ...
    public void invoke(Request request, Response response, ValveContext context) throws IOException, ServletException {
        ...
        SimpleApplicationFilterChain filterChain =
                ApplicationFilterFactory.createFilterChain(sreq, wrapper, servlet);

        filterChain.doFilter(sreq, sres);


        /**
         * 原来servlet直接在WraperValve中执行，现在放到filter chain中执行
         * 等到filter chain中所有filter执行完毕后，再执行servlet
         */
